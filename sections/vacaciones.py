...
from ui_files.vacaciones_ui import Ui_Form
import pandas as pd
from docx import Document
import datetime
import locale

class VacacionesWidget(QWidget):
    def __init__(self):
        super().__init__()
        self.ui = Ui_Form()
        ...
        self.ui.dias_input.valueChanged.connect(self.calculate_new_date)
        ...
        # Set possible periods
        current_perdiods, last_periods = self.calculate_possible_periods()
        self.ui.periodo_input.addItems(current_perdiods)
        self.ui.ultimo_periodo_input.addItems(last_periods)
        ...
        if no_empleado.isdigit():
            ...
            if no_empleado in df['No. DE EMPLEADO'].values:
                try:
                    ...
                    self.ui.adscripcion_input.setText(df.loc[row, 'Adscripción'])
    ...
    def get_input_data(self):
        data = {
            ...
            "Adscripción": self.ui.adscripcion_input.text(),
            "Días a Disfrutar": self.ui.dias_input.value(),
            "Periodo a Disfrutar": self.ui.periodo_input.currentText(),
            "Inicio": self.ui.inicio_input.text(),
            "Termino": self.ui.termino_input.text(),
            "Reincorporación": self.ui.reincorporacion_input.text(),
            "Último periodo": self.ui.ultimo_periodo_input.currentText(),
            "Observaciones": self.ui.observ_input.text()
        }
        ...
        last_period, second_last_period = self.get_two_last_periods()
        data['Último periodo'], data['Último periodo_'] = last_period, second_last_period
    ...
    def get_two_last_periods(self):
        try:
            current_periods, last_periods = self.calculate_possible_periods()
            index = current_periods.index(self.ui.periodo_input.currentText())
        
            ## CURRENT PERIODS = SEGUNDO 2025, PRIMER 2025, SEGUNDO 2024
            ## [0], [1], [2]
            ## LAST PERIODS = PRIMER 2025, SEGUNDO 2024, PRIMER 2024, SEGUNDO 2023
            ## [0], [1], [2], [3]

            # Si cp = 0, entonces lp = 0 y 1
            # Si cp = 1, entonces lp = 1 y 2
            # Si cp = 2, entonces lp = 2 y 3

            # Se extraeron los LAST PERIODS, son los que se muestran en la plantilla
            # Antes de escribirlos, se tiene que quitar la X al que no este en ultimo_periodo_input
            if last_periods[index] != self.ui.ultimo_periodo_input.currentText():
                last_periods[index] = last_periods[index].replace("X", "")
            else:
                last_periods[index + 1] = last_periods[index + 1].replace("X", "")

            return last_periods[index], last_periods[index + 1]
    ...
    def register_data_on_database(self):
        ...
        try:
            df = pd.read_excel("./data/database/vacaciones.xlsx") if pd.io.common.file_exists("./data/database/vacaciones.xlsx") else pd.DataFrame()
        except Exception:
            df = pd.DataFrame()

        # Append the new DataFrame to the existing DataFrame
        df = pd.concat([df, nuevo], ignore_index=True)
        df.to_excel("./data/database/vacaciones.xlsx", index=False)

        self.clear_inputs()
        self.load_recent_records()
    
    def create_word_document(self):
        template = Document('./data/templates/vacaciones.docx')
        data = self.get_input_data()
        for input in data:
            try:
        ...
        # Suggested file name
        suggested_name = f"{data['Apellido Paterno'].upper()}_{data['Apellido Materno'].upper()}_{data['Nombre'].upper()}_VACACIONES.docx"
    ...
    def clear_inputs(self):
        ...
        self.ui.adscripcion_input.clear()
        self.ui.dias_input.setValue(0)
    ...
    def load_recent_records(self):
        ...
        try:
            df = pd.read_excel("./data/database/vacaciones.xlsx")
        except Exception:
    ...
    def calculate_new_date(self):
        days = self.ui.dias_input.value() - 1
    ...
    def calculate_possible_periods(self):
        current_year = datetime.datetime.now().year
        current_month = datetime.datetime.now().month

        # For periodo_input
        if current_month <= 6:
            current_possible_periods = [f"PRIMER PERIODO {current_year}", f"SEGUNDO PERIODO {current_year - 1}", f"PRIMER PERIODO {current_year - 1}"]
        else:
            current_possible_periods = [f"SEGUNDO PERIODO {current_year}", f"PRIMER PERIODO {current_year}", f"SEGUNDO PERIODO {current_year - 1}"]

        ################# ARREGLAR ##################
        # For ultimo_periodo_input
        if current_month <= 6:
            last_possible_periods = [f"SEGUNDO  (X) {current_year - 1}", f"PRIMERO  (X) {current_year - 1}", f"SEGUNDO  (X) {current_year - 2}", f"PRIMERO  (X) {current_year - 2}"]
        else:
            last_possible_periods = [f"PRIMERO  (X) {current_year}",  f"SEGUNDO  (X) {current_year - 1}",f"PRIMERO  (X) {current_year - 1}", f"SEGUNDO  (X) {current_year - 2}"]

        return current_possible_periods, last_possible_periods