...
from ui_files.modif_vacaciones_ui import Ui_Form
import pandas as pd
from docx import Document
import datetime
import locale

class ModifVacacionesWidget(QWidget):
    def __init__(self):
        super().__init__()
        self.ui = Ui_Form()
        ...
        current_perdiods = self.calculate_possible_periods()
    ...
    def get_input_data(self):
        data = {
            ...
            "Motivo": self.ui.motivo_input.toPlainText().upper(),
        }
    ...
    def register_data_on_database(self):
        ...
        try:
            df = pd.read_excel("./data/database/modif_vacaciones.xlsx") if pd.io.common.file_exists("./data/database/modif_vacaciones.xlsx") else pd.DataFrame()
        except Exception:
            df = pd.DataFrame()

        # Append the new DataFrame to the existing DataFrame
        df = pd.concat([df, nuevo], ignore_index=True)
        df.to_excel("./data/database/modif_vacaciones.xlsx", index=False)

        self.clear_inputs()
        self.load_recent_records()
    
    def create_word_document(self):
        template = Document('./data/templates/modif_vacaciones.docx')
        data = self.get_input_data()
        for input in data:
            try:
                ...
                if input == 'Días a Disfrutar':
                    if data[input] == '1':
                        data[input] = data[input] + ' día'
                    else:
                        data[input] = data[input] + ' días'
            except Exception:
                pass
        ...
        # Suggested file name
        suggested_name = f"{data['Apellido Paterno'].upper()}_{data['Apellido Materno'].upper()}_{data['Nombre'].upper()}_MODIFICACION_VACACIONES.docx"
    ...
    def calculate_possible_periods(self):
        ...
        return current_possible_periods